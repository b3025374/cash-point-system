...
template <class ITEM>
List<ITEM>& List<ITEM>::operator=( const List<ITEM>& rhs) {	//copy assignment operator
	if (&rhs != this) 	//to check for self-assignment
    {
    	destroy();		//to release the memory cells currently in (*this)
		copy(rhs);		//to copy the content of rhs on (*this)
    }
    return *this;		//to allow casecaed assignments
}
/*
//using the 'copy & swap' idiom: to be discussed later!
template <class ITEM>
